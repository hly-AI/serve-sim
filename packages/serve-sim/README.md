...
For **physical iOS devices** (USB), see the sibling package [`serve-device`](./packages/serve-device) (`npx serve-device`, default port 4200). Simulator and device CLIs keep separate state directories so they can run side by side.
...
For **physical devices**, use [`serve-device`](../serve-device) (`npx serve-device`) instead — USB streaming + WebDriverAgent touch, isolated from this package's ports/state.
